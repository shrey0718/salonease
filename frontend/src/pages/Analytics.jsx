...
export default function Analytics() {
  ...
  useEffect(() => {
    ...
    fetch("https://salonease-backend-qn1t.onrender.com/product-leads")
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setProductLeads(data);
        else setProductLeads([]);
      })
    ...
    fetch("https://salonease-backend-qn1t.onrender.com/class-leads")
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setClassLeads(data);
        else setClassLeads([]);
      })
      .catch(() => setClassLeads([]));
  ...
  const totalOrdered =
    productLeads.filter(l => l.status === "Ordered").length;

  const conversionRate =
    totalLeads ? ((totalOrdered / totalLeads) * 100).toFixed(1) : 0;
